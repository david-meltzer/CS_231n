...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2*N):
          den_1 += (i!=k)*torch.exp(sim(out[i],out[k])/tau)
          den_2 += (i!=k+N)*torch.exp(sim(out[i],out[k+N])/tau)

        l_1 = -torch.log(num_1/den_1)
        l_2 = -torch.log(num_1/den_2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    den = torch.sqrt(out_left.square().sum(dim=1,keepdim=True))*torch.sqrt(out_right.square().sum(dim=1,keepdim=True))
...
def compute_sim_matrix(out):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    unnorm_prod = torch.matmul(out,torch.transpose(out,0,1))
    norms = out.square().sum(dim=1,keepdims=True).sqrt()
    norm_mat = torch.matmul(norms,torch.transpose(norms,0,1))

    sim_matrix = unnorm_prod/norm_mat
    pass
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = exponential.sum(dim=1,keepdim=True).to(device)
    #print(denom.shape)
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    diag_terms = sim_positive_pairs(out_left, out_right).to(device)
    diag_terms = torch.cat((diag_terms,diag_terms),0)
    pass

    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = torch.exp(diag_terms/tau)
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ...
    loss = -torch.log(numerator/denom).sum()
    loss = loss/(2*N)
