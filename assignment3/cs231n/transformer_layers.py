...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        key = torch.reshape(key,(N,T,H,int(D/H)))
        value = torch.reshape(value,(N,T,H,int(D/H)))
        query = torch.reshape(query,(N,S,H,int(D/H)))
        
        key = torch.permute(key,(0,2,3,1)) #(N,H,D/H,T)
        
        value =  torch.permute(value,(0,2,1,3)) #(N,H,T,D/H)
        
        query = torch.permute(query,(0,2,1,3)) #(N,H,S,D/H)
        ...
        Y = torch.matmul(query,key)

        #print('shape of QK ',Y.shape)
        ...
        if attn_mask != None:
          Y = Y.masked_fill(attn_mask==False,-np.inf)
        ...
        #print('shape of permuted Y ',Y.shape)

        Y = torch.reshape(Y,(N,S,D))

        #print('shape of reshaped Y ',Y.shape)
