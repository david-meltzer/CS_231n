...
def rnn_step_backward(dnext_h, cache):
    ...
    next_h_affine=np.dot(x,Wx)+np.dot(prev_h,Wh)+b

    dtanh = np.cosh(next_h_affine)**-2
    ...
    dx = np.dot(dpt0,Wx.transpose())
    
    dWx= np.dot(np.transpose(x),dpt0)

    dprev_h = np.dot(dpt0,Wh.transpose())

    dWh = np.dot(np.transpose(prev_h),dpt0)
...
def rnn_backward(dh, cache):
    ...
    N,T,H=dh.shape
    placeholder, _,_,_ ,_= cache[0]
    D = placeholder.shape[1]
    ...
    dx = np.zeros((N,T,D))
    dWxpt1 = np.zeros((D,H,T))
    dWhpt1 = np.zeros((H,H,T))
    dbpt1 = np.zeros((H,T))
    dprev_h_pt1= np.zeros((N,H,T))
    ...
    db = np.zeros(H)
    dh0=np.zeros((N,H))
    dprev_h= np.zeros((N,H,T))

    dLtdh=np.zeros((T,T,N,H))

    #prodJacobians=1

    _,h0, Wx, Wh, b = cache[0]

    for t in range(T-1,-1,-1):
      _, dprev_h_pt1[:,:,t], dWxpt1[:,:,t], dWhpt1[:,:,t], dbpt1[:,t] = rnn_step_backward(dh[:,t,:], cache[t])

      x[:,t,:], prev_h[:,t,:], _ , _,_ = cache[t]
      #dx, dprev_h, dWx, dWh, db
      #cache = (x,prev_h,Wx,Wh,b)

    ht2_affine = np.zeros((N,T,H))
    dtanht2 = np.zeros((N,T,H))

    for t1 in range(T):
      for t2 in range(t1,-1,-1):

        ht2_affine[:,t2,:]=np.dot(x[:,t2,:],Wx)+np.dot(prev_h[:,t2,:],Wh)+b
        dtanht2[:,t2,:] = np.cosh(ht2_affine[:,t2,:])**-2
        ...
        if t2==t1:
          ...
          #ht2_affine_p1=np.dot(x[:,t2+1,:],Wx)+np.dot(prev_h[:,t2+1,:],Wh)+b
          #dtanht2_p1 = np.cosh(ht2_affine_p1)**-2
          
          dLtdh[t1,t2,:,:]=np.dot(dLtdh[t1,t2+1,:]*dtanht2[:,t2+1,:],Wh.transpose())

        db += np.sum(dLtdh[t1,t2,:,:]*dtanht2[:,t2,:],axis=0)
        
        dWx += np.dot(np.transpose(x[:,t2,:]),dLtdh[t1,t2,:,:]*dtanht2[:,t2,:]) 
        
        dWh += np.dot(prev_h[:,t2,:].transpose(),dLtdh[t1,t2,:,:]*dtanht2[:,t2,:])
        
        
      
      dh0 += np.dot(dLtdh[t1,0,:,:]*dtanht2[:,0,:],Wh.transpose())
    
    for s2 in range(T):
      for s1 in range(s2,T,1):
        
        dx[:,s2,:] += np.dot(dLtdh[s1,s2,:,:]*dtanht2[:,s2,:],Wx.transpose())
       
       # if t1==0 and t1==t2: 
      #    print(dLtdh.shape)
       #   print(dtanht2.shape)
       #   print((dLtdh[t1,t2,:,:]*dtanht2).shape)
       #   print((np.sum(dLtdh[t1,t2,:,:]*dtanht2,axis=0)).shape)
